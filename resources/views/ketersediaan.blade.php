...
<body>
      ...
      <div class="collapse navbar-collapse justify-content-md-end" id="navbarNavDropdown">
        <ul class="navbar-nav">
          ...
          <li class="nav-item dropdown">
            <a class="nav-link active dropdown-toggle" aria-current="page" href="data" id="navbarDropdownMenuLink"
        ...
        <table class="table table-dark table-striped ">
          ...
          <tbody>
            @foreach ($stocks as $stock)
              <tr>
                <td>{{ $stock->delivery_order }}</td>
                <td>{{ $stock->date_send }}</td>
                <td>{{ $stock->client_slug }}</td>
                <td>{{ $stock->quantity }}</td>
                <td>{{ $stock->kurir_agent }}</td>
                <td>{{ $stock->shipment_number }}</td>
                <td>{{ $stock->tracking }}</td>
                <td>{{ $stock->instansi }}</td>
                <td>{{ $stock->address }}</td>
                <td>{{ $stock->nama_penerima }}</td>
                <td>{{ $stock->no_hp }}</td>
                <td>{{ $stock->staff_user_id }}</td>
                <td>{{ $stock->sender_user_id }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
  ...
  <footer class="text-center"
    style=" background-image: linear-gradient(to right, rgb(0, 0, 0) , rgb(0, 0, 0) , rgb(38, 10, 2));">
